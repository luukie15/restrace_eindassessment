$(document).ready(function () {
    // fills waypoint table ( waypoints of race )
    fillWaypointTable();
    // fills user table ( users of race )
    fillUserTable();
    // gets info of a single waypoint if user clicks on waypoint
    doSearchForCafes();
    // admin can start race
    doAdminStartRace();
    // admin can stop race
    doBtnStopRace();
    // switch on userrole
    switch ($("#userrole").val()) {
        case 'admin':
            // if admin, hide the functionality to gain information about a waypoint (it's already in /waypoints)
            $(".waypoint_info_races").hide();
            break
        case 'visitor':
        case 'moderator':
            // for the visitor and the moderator role (don't have moderator but yeah...), switch information based on status of race
            ...
            break;
        default:
            // if 'geen rol' is the role of the user (when not logged in), pass link to login
    ...
    $(".btn_delete_waypoint").each(function () {
        $(this).click(function () {
            ...
            $.ajax({
                ...
                if (response.msg === '') {
                    //$(".waypoints_of_race").html("");
                    $(".waypoints_of_race").find('button[rel="' + reference +'"]').parent().remove();
